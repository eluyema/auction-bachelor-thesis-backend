...
describe('DefaultAuctionStrategy', () => {
    let strategy: DefaultAuctionStrategy;
    ...
    beforeEach(() => {
        ...
        strategy = new DefaultAuctionStrategy(mockAuction);
    });

    describe('makeBid', () => {
        test('should process bid and return updated rounds', async () => {
            const makeBidDto: MakeBidDto = { total: 12000000 };
            ...
            expect(
                result.some((round) => round.Bids.some((bid) => bid.total === BigInt(12000000))),
            ).toBeTruthy();
        });

        test('the smallest bid must be last in next round', async () => {
            const smallBid: MakeBidDto = { total: 12000 };
            const smallBidUser = '1c5934d6-f63f-4d3b-bc8a-0b5b927ef0ee';

            const bigBid: MakeBidDto = { total: 1200000000000 };
    ...
    describe('createInitialBid', () => {
        test('should successfully create initial bid and update rounds', async () => {
            const createBidDto: CreateInitialBidDto = { total: 15000000 };
            const result = await strategy.createInitialBid(createBidDto, 'newUserId');
            expect(
                result.some((round) => round.Bids.some((bid) => bid.total === BigInt(15000000))),
            ).toBeTruthy();
        });
    });

    describe('removeUserFromRounds', () => {
        test('should remove user bids from rounds', async () => {
